...
exports.searchCast = functions.runWith({ secrets: ['MOVIEDB_API_KEY'], maxInstances: 10 }).https.onCall((data, context) => {
...
exports.searchMovie = functions.runWith({ secrets: ['MOVIEDB_API_KEY'], maxInstances: 10 }).https.onCall((data, context) => {
...
exports.getCastForMovieId = functions.runWith({ secrets: ['MOVIEDB_API_KEY'], maxInstances: 10 }).https.onCall((data, context) => {
  return fetch(`${movieApiUrl}/movie/${data.movieId}/credits?api_key=${process.env.MOVIEDB_API_KEY}`).then((response) => {
    return response.json();
  });
});

exports.getMovieForPersonId = functions.runWith({ secrets: ['MOVIEDB_API_KEY'], maxInstances: 10 }).https.onCall((data, context) => {
